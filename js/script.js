let menu = [
	{ id: 1, nama: 'Indomie Telur Keju Kornet', kategori: 'food', harga: 18000, foto: 'interjunet.png'},
	{ id: 2, nama: 'Roti Bakar Coklat', kategori: 'food', harga: 10000, foto: 'ropang.png'},
	{ id: 3, nama: 'Kue Pancong', kategori: 'food', harga: 8000, foto: 'pancong.png'},
	{ id: 4, nama: 'Es Jeruk', kategori: 'drink', harga: 5000, foto: 'esjeruk.png'},
	{ id: 5, nama: 'Kopi Susu', kategori: 'drink', harga: 3000, foto: 'kopisusu.png'}
]
...
function loadCart(){
	...
	if (localStorage.cart){
		...
		for (i in cart){
			...
			var total_bayar = total_cart
			$("#total-bayar").html(formatRupiah(total_bayar))
		}
	} else {
		$("#cart-info").hide()
		data_cart += `<div class="alert alert-primary">Keranjang anda masih kosong.</div>`
		$("#cart-content").html(data_cart)
	}
...
function loadOrder(){
	...
	if (localStorage.order){
		...
		data_order += `<div class="alert alert-primary m-2">Belum ada pesanan dari anda</div>`
	}
	...
	if (localStorage.cart){
		...
		for (i in cart){
			if(cart[i].id == id_menu){
				$("#modal-message").html('Menu sudah ada di dalam keranjang')
				$("#modalAlert").modal('show')
				return false
			}
		}
...
function addOrder(){
	...
	if (localStorage.cart){
		...
		var total_bayar = total_pesanan
		msg_order += ' total harga pesanan Rp ' + formatRupiah(total_bayar) + '. Terima kasih banyak!'
...
function showOrder(id){
	...
	var total_bayar = total_order_detail
...
function liffOpenWindow(){
	if (!liff.isInClient()) {
		...
		liff.openWindow({
	        url: 'https://liff.line.me/1655321390-Ro94GEOj',
	        external: true
	    })
	}
}
